(function () {
    ...
    function addPicture(attr) {
        ...
        img += 'class="' + attr.id + ' paintbot-pic" ';
    ...
    function changeElem(cmd) {
        var elem, newPos = {}, i = 0, len = cmd.element.length, transform, deltaX, deltaY;
        if (cmd.action & MOVE) {
            for (; i< len; i += 1) {
                elem = cmd.element[i];
                transform = paintbot.svg.createSVGTransform();
                elem.transform.baseVal.appendItem(transform);
                deltaX = deltaX ? deltaX : cmd.x - elem.x.baseVal.value;
                deltaY = deltaY ? deltaY : cmd.y - elem.y.baseVal.value;
                elem.transform.baseVal.getItem(0).setTranslate(deltaX, deltaY);
            }
        }
    }
        ...
        loadData(url,'text').then(function (response) {
            ...
            if (typeof response !== 'string' || response === '') {
                ...
                if (paintCmd === '') {
                    //alert('Keine weiteren Zeichenanweisungen vorhanden.')
                    //return;
                } else if (typeof paintCmd === 'string') {
            ...
            //paintCmds.documentElement.children.length;
            window.setTimeout(loadCommands, timeOut);
        }, function (response) {
            /*var cmd = {
             "identifier": "d83a9600-bb48-409f-b2a6-c1f679f58d7c",
             "tagname": "baum",
